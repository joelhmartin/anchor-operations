...
| F4 | Recommendation → action engine | `2026-06-28-f4-recommendation-action-engine.md` | **in_review** | feat/ops-f4-recommendation-action-engine / PR #15 | 13 tasks, 56 new tests (10 test files). 249/263 pass; 14 pre-existing baseline failures unchanged. Deterministic-first: all numbers in pure JS before single LLM call. HIPAA gate preserved (medical → escalate approval level, client_type never echoed). Destructive actions blocked. F1/F3 stubs degrade gracefully. |

...
| F2 | Inventory discovery | `2026-06-28-f2-inventory-discovery.md` | **in_review** | PR #13, branch `feat/ops-f2-inventory-discovery` | 9 tasks, 23 new F2 tests. 140/154 pass; 14 pre-existing baseline failures unchanged. RECONCILE applied: F1's table uses `attributes_json`/`discovered_at`; F2 adds ALTER TABLE for status/parent_external_id/url + drops NOT NULL on client_user_id. inventoryStore seeds ops_service_connections FK. |
